 ...
 * @author alyssa
 */
public class GenericCounterpoint_w_keychange_3 {
...
static Integer [] consonances = InputParameters.consonances;
static Integer [] perfect_consonances = InputParameters.perfect_consonances;
static Integer [] root_consonances = InputParameters.root_consonances;
...
static int tempo_bpm = InputParameters.tempo_bpm;
...
static int piece_length = InputParameters.piece_length;
static int root_key = 0;
static ModeModule my_mode_module = InputParameters.my_mode_module;
static String [] voice_array = InputParameters.voice_array;  
static int key_transpose = 0;
static RhythmModule james = InputParameters.james;
...
static Boolean large_dissonance_bad = InputParameters.large_dissonance_bad;
static ArrayList<LinkedList> built_voice_queues = new ArrayList();
                            ...
                            if (!skip_me){
                                    ...
                                    if (previous_cf_pitch[b]!=1111)
                                        pitch_candidates = harmonicChecks(pitch_candidates, this_cf, CF_root, previous_cf_pitch[b],
                                                            previous_cp_pitch, CP_note, voice_pitch_count, b);
                                    else pitch_candidates = harmonicChecksSuperBasic(pitch_candidates, this_cf, CF_root, alter_me.getMelodicNote(current_cp_index));
                                   previous_cp_pitch =9999; // 9999 is assigned in case CP might hold over into next CF note
...
*/	
    ...
    public static class PitchCandidate {
        Integer rank = 0;
        Integer pitch = 0;
        Integer[] latest_interval_wCF = new Integer[built_voice_queues.size()];
        ...
        public void setLatestInterval (Integer interval, int cf_voice_index) {
            latest_interval_wCF[cf_voice_index] = interval;
        
        }
        
        public Integer getLatestInterval (int cf_voice_index) {
            return latest_interval_wCF[cf_voice_index];
        }
    }
    ...
    public static ArrayList<PitchCandidate> harmonicChecks(ArrayList<PitchCandidate> pitch_candidates, MelodicNote CF_note, Boolean CFnoteRoot, Integer previous_cf_pitch,
                                                        Integer previous_cp_pitch, MelodicNote CP_note, int voice_pitch_count, int cf_voice_index ){
            ...
            for (PitchCandidate myPC : pitch_candidates){
                ...
                if(previous_cp_pitch == 9999) previous_cp_pitch = cand_pitch;// 9999 means the CP is held over to multiple cfs
                Integer melody_motion_to_cand = cand_pitch  - previous_cp_pitch;
                int this_interval = abs(myPC.getPitch() - CF_note.getPitch())%12;
                //if (myPC.getLatestInterval(cf_voice_index) != null) previous_cf_pitch = myPC.getLatestInterval(cf_voice_index); //NEW
